...
user_data = {"username": "agent007", "email": "agent007@gmail.com", "password": "1234567"}
...
def test_repeat_signup(client, monkeypatch):
    ...
    # Спершу створіть користувача, щоб переконатися, що він вже існує
    response = client.post("api/auth/signup", json=user_data)
    assert response.status_code == 201, response.text  # Переконайтеся, що користувач був успішно створений

    # Повторна спроба реєстрації того ж користувача
    response = client.post("api/auth/signup", json=user_data)
