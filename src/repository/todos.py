...
async def create_todo(body: TodoSchema, db: AsyncSession, user: User):
    todo = Todo(**body.model_dump(
        exclude_unset=True), user=user)  # model_dump - замість dict , exclude_unset - означає, якщо є пусте поле, то його треба викинути
    # ** означає key-value (title=body.title, description=body.description)
    db.add(todo)
    await db.commit()
