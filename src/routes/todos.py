...
access_to_route_call = RoleAccess([Role.admin, Role.moderator])
...
@router.get("/all", response_model=list[TodoResponse], dependencies=[Depends(access_to_route_call)])
...
async def create_todo(body: TodoSchema, db: AsyncSession = Depends(get_db), user: User = Depends(
    auth_service.get_current_user)):  # user: User = Depends(auth_service.get_current_user)
...
async def delete_todo(todo_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(
    auth_service.get_current_user)):  # user: User = Depends(auth_service.get_current_user)
